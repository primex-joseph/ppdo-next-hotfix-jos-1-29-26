...
export function TemplateSelector({ isOpen, onClose, onCloseAll, onSelectTemplate }: TemplateSelectorProps) {
  ...
  return (
    <ResizableModal open={isOpen} onOpenChange={(open) => !open && handleCloseButton()}>
      <ResizableModalContent
        width="1200px"
        height="800px"
        maxWidth="95vw"
      ...
      >
        {/* Header - Simplified without orientation selector */}
        <ResizableModalHeader>
        ...
        <ResizableModalBody className="px-6 py-6">
          {templates.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {/* Create New Template Card - Only show when templates exist */}
              <button
              ...
              >
                <div className="aspect-[8.5/11] flex flex-col items-center justify-center p-6 text-center">
                  ...
                  <h4 className="font-semibold text-base text-stone-900 dark:text-stone-100 mb-2">
                    Create New Template
                  </h4>
                  <p className="text-sm text-stone-500 dark:text-stone-400 leading-tight">
                    Open canvas editor to design a custom template
                  </p>
                </div>
              </button>
              ...
              >
                  ...
                  {hoveredId === template.id && !template.isDefault && (
                    <div className="absolute top-3 right-3 flex gap-2 z-10">
                      <button
                        ...
                        className="p-2 bg-white dark:bg-stone-800 rounded-md shadow-lg hover:bg-stone-100 dark:hover:bg-stone-700 transition-colors"
                      ...
                      <button
                        ...
                        className="p-2 bg-white dark:bg-stone-800 rounded-md shadow-lg hover:bg-red-50 dark:hover:bg-red-950 transition-colors"
                ...
                {selectedId === template.id && (
                  <div className="absolute top-3 left-3 bg-[#15803D] text-white rounded-full p-1 shadow-lg z-10">
                    <Check className="w-4 h-4" />
                  </div>
                )}

                {template.isDefault && (
                  <div className="absolute top-3 left-3 px-2 py-1 bg-[#15803D] text-white text-xs font-medium rounded z-10">
                    Default
                  </div>
                )}

                <div className="p-4 bg-white dark:bg-stone-800 border-t border-stone-200 dark:border-stone-700">
                  <h4 className="font-semibold text-base truncate text-stone-900 dark:text-stone-100 mb-1">
                    {template.name}
                  </h4>
            ...
            /* Empty State - No "Create New Template" card here */
            <div className="text-center py-16">
              <div className="w-20 h-20 rounded-full bg-[#15803D]/10 flex items-center justify-center mx-auto mb-4">
                <FileText className="w-10 h-10 text-[#15803D]" />
              </div>
              <h3 className="text-lg font-semibold text-stone-900 dark:text-stone-100 mb-2">
                No templates available
              </h3>
              <p className="text-stone-500 dark:text-stone-400 mb-6">
                Create your first template to get started with custom designs
              </p>
        ...
        <ResizableModalFooter className="px-6 py-4 bg-stone-50 dark:bg-stone-800/50 flex-row justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              disabled={!selectedId}
              type="button"
              className="bg-[#15803D] hover:bg-[#15803D]/90 disabled:bg-stone-300 disabled:text-stone-500"
            >
