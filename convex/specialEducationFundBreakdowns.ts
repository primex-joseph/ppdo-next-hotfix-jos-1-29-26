...
export const bulkMoveToTrash = mutation({
  args: {
    ids: v.array(v.id("specialEducationFundBreakdowns")),
    reason: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
    const userId = await getAuthUserId(ctx);
    ...
    // Recalculate SEF Metrics once after all deletions
    if (results.success > 0 && args.ids.length > 0) {
      const firstBreakdown = await ctx.db.get(args.ids[0]);
      if (firstBreakdown) {
        await recalculateFundMetrics(ctx, firstBreakdown.specialEducationFundId, "specialEducationFunds", userId);
      }
    }
